...
export async function GET(request: Request) {
    console.log('GET session-sync/route.ts');
  const { searchParams } = new URL(request.url);
  const jwt = searchParams.get('jwt');

  if (!jwt) {
    console.log('GET session-sync/route.ts - redirect as no jwt');
    return redirect('/login?error=MissingToken');
  }

  console.log('GET session-sync/route.ts - jwt', jwt);
  ...
  try {
    ...
    console.log('GET session-sync/route.ts - response', response);
    ...
    // Restore the customer session - bm version
    // (signIn will re-validate an active session for next-auth)
    if (data.content?.customerAccessToken?.value && data.content?.customer?.email) {
      await signIn('credentials', {
        type: 'cat',
        cat: data.content.customerAccessToken.value,
        email: data.content.customer.email,
        firstName: data.content.customer.firstName,
        lastName: data.content.customer.lastName,
        redirectTo: data.content.redirectTo,
      });
    }

    //  // Restore the customer session - njb
    // // (signIn will re-validate an active session for next-auth)
